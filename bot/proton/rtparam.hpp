...
public:
	...
	{ //this does not chekc if it exists, it assumes validate_ints has been consulated beforehand
		return atoi(find(key)->m_value.c_str());
	}
	inline long long get_long(const std::string &key)
	{ //assumes validate_ints
...
class rtvar_opt
{ //optimized version of rtvars (really nothing more than a container) when only needing to append strings
private:
	std::string m_var{};
